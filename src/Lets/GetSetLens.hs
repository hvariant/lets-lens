...
setL ::
  ...
  k
  -> Lens (Set k) Bool
setL k = Lens s (Set.member k)
  where s m True = Set.insert k m
        s m False = Set.delete k m
...
compose ::
  ...
  -> Lens a b
  -> Lens a c
compose (Lens sbc gbc) (Lens sab gab) = Lens s (gbc . gab)
  where s a = sab a . sbc (gab a)
...
choice (Lens sax gax) (Lens sbx gbx) = Lens s g
  where g (Left a) = gax a
        g (Right b) = gbx b
        s (Left a) x = Left $ sax a x
        s (Right b) x = Right $ sbx b x
